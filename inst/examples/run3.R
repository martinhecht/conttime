...
# library( conttime ) # 0.0.36 2024-05-12
...
design.env <- gen.design(F=2,Tdiv=-1)
...
# stn <- gen.stan( data.env=data.env, KF=FALSE ) # , start.values.env=sv.env
...
# hist(est$bias)
est <- est[ order( abs( est$bias.percent ), abs( est$bias ), decreasing=TRUE ), ]
